...
export function parseExcelBuffer(buf: Buffer): ParsedRow[] {
  ...
  for (const row of rawRows) {
    ...
    const creatorRaw = parseStrOrNull(get("creatorUsername"));
    const creator = creatorRaw ? creatorRaw.toLowerCase().replace(/^@/, "").trim() : "";
  ...
  return out.filter((r) => r.orderId && r.creatorUsername);
}
